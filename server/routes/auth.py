...
def register():
    ...
    #     user_exists = session.query(Applicant).filter(Applicant.email == req["email"]).all()
    # except:
    #     response = jsonify({"code": "-1", "message": "Couldn't Query SQL database for user "+req["email"]+" of user type Applicant!"})
    #     return make_response(response, 401)
    ...
    if req["recruiter"]:                                                                                                                            
        ...
        attr = ['email', 'name', 'school', 'year', 'gpa', 'location', 'picture_name']
        data = [(req['email'], '-', '-',5,3.91,'-','-')]
        user_type = Applicant
    try:
        add_object_to_db(attr, data, user_type)
    except:
        response = jsonify({"code": "-1", "message": "Couldn't add User to DB, user might exist!"})
        return make_response(response, 401)
